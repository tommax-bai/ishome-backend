package com.ishome.channel.testsupport;
...
/** 持久化集成测试装配：独立 schema {@code svc_channel_it}（与开发数据互不污染）+ 每次测试上下文启动时 clean → migrate（测试间互不污染）。 */
public final class PostgresIntegrationTestSupport {

  /** schema-per-service 的测试位：正式 schema svc_channel 不被集成测试触碰。 */
  public static final String SCHEMA = "svc_channel_it";
